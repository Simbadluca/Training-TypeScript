...
describe('about let and const', () => {
  it('1-should be only available in a if block scope', () => {
    var myVar: number = 1;  //global
    let myLet: number = 2;  //local
  ...
  it('2-should be only available in a for block scope', () => {
    let forLet: number = 0; // local

    for (var forVar = 0; forVar < 10; forVar++) {
      for (let forLet = 0; forLet < 10; forLet++) {  // aussi local donc comme si 2 variables differentes
        //do something
      }
    }
  ...
  it('3-should solves some async issues', (done) => {
    ...
    setTimeout(() => {
      expect(varStack).to.eql( 3 ); // 3,3,3
      expect(letStack).to.eql( 2 ); // 0,1,2
      done();
    });
  });
